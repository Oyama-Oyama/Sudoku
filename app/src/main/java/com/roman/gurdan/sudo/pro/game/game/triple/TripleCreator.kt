package com.roman.gurdan.sudo.pro.game.game.triple
 ...
 *
 */
internal class TripleCreator(gameSize: GameSize) : ISpliceCreator(gameSize) {
    ...
    override fun getCellAreas(row: Int, col: Int): Array<Int> {
        if (row in 6..8 && col in 6..8) return arrayOf(AREA_FIRST, AREA_SECOND)
    ...
    override fun getCellArea(row: Int, col: Int): Int {
        if (row in 9..14 && col in 9..11) return -1
