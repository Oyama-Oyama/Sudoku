...
//        super.onBackPressed();
  ...
  protected void giveUp() {
    if (boardView.isGameOver()) {
      ...
      try {
        ...
        bundle.putString("size", game.getGameSize().getValue() + "");
        bundle.putString("difficulty", game.getGameDifficulty()._difficulty + "");
        Easy.Companion.getInstance().logEvent("giveUpGame", bundle);
      } catch (Exception e) {
      }
    }
  }
  ...
  protected void resetGame() {
    new AlertDialog.Builder(this)
      ...
      .setPositiveButton(R.string.sure, new DialogInterface.OnClickListener() {
        @Override
        public void onClick(DialogInterface dialogInterface, int i) {
          if (Easy.Companion.getInstance().hasInterstitial()) {
            Easy.Companion.getInstance().addInterstitialListener(new IAdListener() {
              @Override
              ...
              @Override
              public void onShowFail(@Nullable String s) {
                realResetGame();
              }
  ...
  private void realResetGame() {
    try {
      ...
      bundle.putString("size", game.getGameSize().getValue() + "");
      bundle.putString("difficulty", game.getGameDifficulty()._difficulty + "");
      Easy.Companion.getInstance().logEvent("resetGame", bundle);
    } catch (Exception e) {
    }
    if (boardView != null) {
      boardView.resetGame();
  ...
  protected void onGameOver() {
    ...
    try {
      ...
      bundle.putString("size", game.getGameSize().getValue() + "");
      bundle.putString("difficulty", game.getGameDifficulty()._difficulty + "");
      Easy.Companion.getInstance().logEvent("win", bundle);
    } catch (Exception e) {
    }
  }
...
//        FrameLayout banner = findViewById(R.id.bottomBanner);
    Easy.Companion.getInstance().showBanner(null);

    Log.e("testevent", "tttttttttt");
    println("tetttttsstttt");
  }
      ...
      Observable.zip(Observable.just(gameSize), Observable.just(difficulty),
        ...
        .doOnError(new Consumer<Throwable>() {
          ...
          public void accept(Throwable throwable) throws Throwable {
            ...
            try {
              Bundle bundle = new Bundle();
              bundle.putString("reason", throwable.getMessage());
              Easy.Companion.getInstance().logEvent("gameCreateError", bundle);
            } catch (Exception e) {
            }
          }
        })
        .subscribe(new Consumer<Game>() {
          ...
          public void accept(Game game) throws Throwable {
            ...
            try {
              ...
              bundle.putLong("createDuration", duration);
              Easy.Companion.getInstance().logEvent("gameCreated", bundle);
            } catch (Exception e) {
            }
          }
        })
    ...
    try {
      if (integer == 0) {
        ...
        if (boardView.hasUndo()) {
          boardView.Undo();
          Easy.Companion.getInstance().logEvent("undo", null);
        } else {
        ...
        Easy.Companion.getInstance().logEvent("notes", null);
      } else if (integer == 3) {
