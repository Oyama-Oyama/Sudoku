...
class CalendarAnimator(private val day: Day, private val initPagerIndex:Int) {
    ...
    suspend fun animateTo(
        ...
        val initialPage = initPagerIndex
        val currentDate =
            when (mode) {
                AnimationMode.MONTH ->
                    day.plus(
        ...
        val targetDate =
            target.run {
                if (mode == AnimationMode.MONTH) {
                    copy(dayOfMonth = currentDate.dayOfMonth)
                } else {
                    this
                }
            }
